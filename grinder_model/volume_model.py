...
def train_and_select_best_model(grind_data, target_columns):
    ...
    for i, (X_train, X_test, y_train, y_test, scaler) in enumerate(bootstrap_samples):
        ...
        model = train_polynomial_regression_with_grid_search(X_train, y_train.drop(columns=['index']))
...
def get_polynomial_coefficients(model):
    # Access PolynomialFeatures and LinearRegression steps in the pipeline
    ...
    print(feature_names)
    # Retrieve the coefficients from the linear regression model
    coefficients = linear_reg.coef_.flatten()
...
def main():
    ...
    #input ['grind_time', 'avg_rpm', 'avg_force', 'grind_area', 'initial_wear']
    get_polynomial_coefficients(best_model)
